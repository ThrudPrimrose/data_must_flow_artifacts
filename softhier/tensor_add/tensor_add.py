...
@dace.program
def tensor_addition(A: dace.float32[Z, Y, X] @ dace.dtypes.StorageType.GPU_Global,
                    B: dace.float32[Z, Y, X] @ dace.dtypes.StorageType.GPU_Global,
                    C: dace.float32[Z, Y, X] @ dace.dtypes.StorageType.GPU_Global):
    for z, i, j in dace.map[0:Z:1,
                         0:Y:(BLOCK_Y * CORES_Y * NUM_BLOCKS_Y),
        ...
        for c_i, c_j in dace.map[i:i + (BLOCK_Y * CORES_Y * NUM_BLOCKS_Y):(BLOCK_Y * NUM_BLOCKS_Y),
            ...
            for  b_i, b_j in dace.map[c_i:c_i + (BLOCK_Y * NUM_BLOCKS_Y):1,
                    ...
                    C[z, b_i, b_j] = A[z, b_i, b_j] + B[z, b_i, b_j]

@dace.program
def tensor_addition_cpu(A: dace.float32[Z, Y, X] @ dace.dtypes.StorageType.CPU_Heap,
                        B: dace.float32[Z, Y, X] @ dace.dtypes.StorageType.CPU_Heap,
                        C: dace.float32[Z, Y, X] @ dace.dtypes.StorageType.CPU_Heap):
    for z, i, j in dace.map[0:Z:1,
                         0:Y:1,
...
def create_data_and_handlers(Z_val, M_val, N_val, hw_config: HardwareConfig):
    ...
    A_host = np.random.uniform(0.5, 1.0, (Z_val * M_val, N_val)).astype(DTYPE_INPUT)
    B_host = np.random.uniform(0.5, 1.0, (Z_val * M_val, N_val)).astype(DTYPE_INPUT)
    C_host = np.ones((Z_val * M_val, N_val), dtype=DTYPE_OUTPUT)
    print(C_host, C_host.shape)

    A_handler = InterleaveHandler(array=A_host, block_shape=(Z_val * M_val, N_val), cluster_dims=hardware_thread_group_dims)
    A_handler.split_to_blocks(tile_dims=(Z_val * M_val, N_val))
    A_handler.place_to_range(place_range=(0, 0, 1))

    B_handler = InterleaveHandler(array=B_host, block_shape=(Z_val * M_val, N_val), cluster_dims=hardware_thread_group_dims)
    B_handler.split_to_blocks(tile_dims=(Z_val * M_val,N_val))
    B_handler.place_to_range(place_range=(0, 0, 1))

    C_handler = InterleaveHandler(array=C_host, block_shape=(Z_val * M_val, N_val), cluster_dims=hardware_thread_group_dims)
    C_handler.split_to_blocks(tile_dims=(Z_val * M_val,N_val))
...
def run_sdfg_in_tempdir(combo, interleavers: Dict[str, InterleaveHandler], hw_config: HardwareConfig,
    ...
    """
    (Z_val, M_val, N_val, ) = combo
    log_file = open("./log", "a")
    log_file.close()
    execution_period_ns = None

    print("[Pipeline Info] Make Preload ELF Interleaved New")
    ...
    make_preload_elf_hbm_interleaved_new("output.elf",
                                         ...
                                         args_only=False)
    print("[Pipeline Info] Made Preload ELF Interleaved New")

    sdfg = softhier_sdfg
    sdfg.validate()

    print("[Pipeline Info] Generate Code for SoftHier SDFG")
    sdfg.generate_code()
    print("[Pipeline Info] Compile SoftHier SDFG")
    compiled_sdfg = sdfg.compile()
    print("[Pipeline Info] Run SoftHier SDFG")
    compiled_sdfg(A=host_data["A"], B=host_data["B"], C=host_data["C"])
    sys.stdout.flush()
    sys.stderr.flush()
...
def _get_softhier_sdfg() -> dace.SDFG:
    ...
    CollapseDimensions(collapse_map={"A": (0, 1), "B": (0, 1), "C": (0, 1)}).apply_pass(sdfg, {})
    ...
    VectorizeSoftHier(vector_width=1024, insert_copies=True, eliminate_trivial_vector_map=True).apply_pass(copy_sdfg, {})
...
if __name__ == "__main__":
    ...
    combo = (Z, M, N)
    print("Create Data Handlers")
    data_and_interleavers = create_data_and_handlers(Z, M, N, config)

    data = data_and_interleavers["numpy_data"]
    interleavers = data_and_interleavers["interleavers"]

    cpu_sdfg = tensor_addition_cpu.to_sdfg()
    from dace.transformation.layout.collapse_dimension import CollapseDimensions
    CollapseDimensions(collapse_map={"A": (0, 1), "B": (0, 1), "C": (0, 1)}).apply_pass(cpu_sdfg, {})

    softhier_sdfg = _get_softhier_sdfg()
    cpu_sdfg.replace_dict({"X": 1024, "Y": 4, "Z": 2})
