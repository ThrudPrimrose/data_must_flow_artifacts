...
def plot_scatter(folder):
    ...
    for fu in [32, 64, 128]:
        ...
        color_map = {pair: colors[i % len(colors)] for i, pair in enumerate(bank_pairs)}
        #print({(i,pair) for i, pair in enumerate(bank_pairs)})
        #print({pair: colors[i % len(colors)] for i, pair in enumerate(bank_pairs)})
        #raise Exception(color_map)
        ...
        for _, row in df_max.iterrows():
            ...
            plt.scatter(
                row['Num Core Per Cluster'],
                row["Performance % of Roofline"],
                color=color_map[(row["Bank Num"], row["Bank Width"])],
        ...
        plt.xlabel("Number of Scalar Cores")
        plt.ylabel("Performance (% to Roofline of a Single Vector Unit)")
        plt.title(f"Scalar Fallback Performance\nFunctional Units={row['Function Units']}, VLSU Ports={row['VLSU Ports']}")
        plt.grid(visible=True, which="both", axis="both", linestyle="--", color="gray", linewidth=0.8)
        plt.xscale("log")
        plt.ylim(-0.1, 10.1)
        ...
        handles_marker = [
            plt.Line2D(
                ...
                color=color_map[bank_num, bank_width],
                linestyle='None',
                markersize=8,
        ...
        all_handles =  handles_marker
        plt.legend(handles=all_handles, loc='lower center', bbox_to_anchor=(0.5, -0.15),
