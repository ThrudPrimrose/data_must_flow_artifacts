...
int main(int argc, char **argv) {
    ...
    arr = aligned_alloc(64, N * sizeof(float));
#else
    /* fallback */
    ...
    /* initialize with some positive values (>0) */
    for (size_t i = 0; i < N; ++i) {
        arr[i] = (float)(i + 1) * 1.0f / 3.0f + 0.125f; /* avoid zero/neg */
    }

    /* run loop and accumulate the results to prevent optimizing away */
    for (size_t i = 0; i < N; i += 8) {
        #pragma GCC ivdep
        #pragma GCC unroll 8
        #pragma GCC vectorize(enable)
        for (int j = i; j < j + 8; j ++){
            arr[j] = dace_log_f(arr[j]);

        }   
    }

    /* print a single value so program isn't optimized out */
    printf("arr = %f\n", (float)arr[0]);
...
riscv32-unknown-elf-objdump -d -S -M no-aliases log_exe > dump.s
*/