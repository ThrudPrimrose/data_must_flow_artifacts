...
def _gpu_to_softhier(sdfg: dace.SDFG):
    ...
    # After logansion, run another pass of connector/type inference
    infer_types.infer_connector_types(sdfg)
    infer_types.set_default_schedule_and_storage_types(sdfg, None)
...
@dace.program
def matrix_logarithm(A: dace.float32[Y, X] @ dace.dtypes.StorageType.GPU_Global,
    ...
    for i, j in dace.map[0:Y:(BLOCK_Y * CORES_Y * NUM_BLOCKS_Y),
        ...
        for c_i, c_j in dace.map[i:i + (BLOCK_Y * CORES_Y * NUM_BLOCKS_Y):(BLOCK_Y * NUM_BLOCKS_Y),
            ...
            for b_i, b_j in dace.map[c_i:c_i + (BLOCK_Y * NUM_BLOCKS_Y):1,
                    ...
                    B[b_i, b_j] = log(A[b_i, b_j])

@dace.program
def matrix_logarithm_cpu(A: dace.float32[Y, X] @ dace.dtypes.StorageType.CPU_Heap,
...
if __name__ == "__main__":
    ...
    cpu_sdfg = matrix_logarithm_cpu.to_sdfg()
