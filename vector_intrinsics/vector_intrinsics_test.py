...
def run_vectorization_test(
    ...
    for _ in range(runs):
        ...
        write_runtime(
            ...
            filename="vector_intrinsics_runtimes"
        )
...
def select_env_flags():
    ...
    if feats.get("sve2", False) or feats.get("sve", False):
        configs.append({"__DACE_USE_INTRINSICS": "1", "__DACE_USE_SVE": "1", "__DACE_USE_AVX512": "0"})

        # Also test NEON
        configs.append({"__DACE_USE_INTRINSICS": "1", "__DACE_USE_SVE": "0", "__DACE_USE_AVX512": "0"})
...
INTRINSIC_FUNCS = [
    ...
    # ------------------------------------------------------------------
    # Unit-stride & gather/scatter patterns
    # ------------------------------------------------------------------
...
@pytest.mark.parametrize("config", select_env_flags())
def test_indexed_gather(config: List[Dict[str, str]]):
    for k, v in config.items():
        os.environ[k] = v
    # -------------------------
    # Prepare test arrays
...
@pytest.mark.parametrize("config", select_env_flags())
def test_indexed_store(config: List[Dict[str, str]]):
    for k, v in config.items():
        os.environ[k] = v
    # -------------------------
    # Prepare test arrays
