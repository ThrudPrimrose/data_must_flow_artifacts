...
def ice_supersaturation_adjustment_dace(
    ...
    # NOTE: same convention as before: species_i, species_j, jl
    zsolqa: dace.float64[nclv, nclv, klon],
    zsolac: dace.float64[klon],
    # species dimension in second axis, jl last
    zqxfg: dace.float64[nclv, klon],
    rtt: dace.float64,
    ramin: dace.float64,
    rthomo: dace.float64,
    nssopt: dace.int32,
    rkoop_tau: dace.float64,
    ptsphy: dace.float64,
    zepsec: dace.float64,
    ncldql: dace.int32,
    ncldqi: dace.int32,
    ...
    # Main loop: Python 0-based indices, Fortran KIDIA..KFDIA is 1-based.
    for jl in range(klon):
        ...
        if ztp1[jl] >= rtt or nssopt == 0:
            ...
            zfaci = ptsphy / rkoop_tau

        if za[jl] > 1.0 - ramin:
            zsup_sat = max(
                (zqx_ncldqv[jl] - zfac * zqsice[jl]) / zcorqsice[jl],
                0.0,
            )
        else:
            zqp1env = (zqx_ncldqv[jl] - za[jl] * zqsice[jl]) / max(
                1.0 - za[jl], zepsilon
            )
            zsup_sat = max(
        ...
        if zsup_sat > zepsec:
            ...
            if ztp1[jl] > rthomo:
                ...
                zsolqa[ncldqv - 1, ncldql - 1, jl] = (
                    zsolqa[ncldqv - 1, ncldql - 1, jl] + zsup_sat
                )
                zsolqa[ncldql - 1, ncldqv - 1, jl] = (
                    zsolqa[ncldql - 1, ncldqv - 1, jl] - zsup_sat
                )
                zqxfg[ncldql - 1, jl] = zqxfg[ncldql - 1, jl] + zsup_sat
            else:
                # cold case: ice
                zsolqa[ncldqv - 1, ncldqi - 1, jl] = (
                    zsolqa[ncldqv - 1, ncldqi - 1, jl] + zsup_sat
                )
                zsolqa[ncldqi - 1, ncldqv - 1, jl] = (
                    zsolqa[ncldqi - 1, ncldqv - 1, jl] - zsup_sat
                )
                zqxfg[ncldqi - 1, jl] = zqxfg[ncldqi - 1, jl] + zsup_sat
...
def generate_ice_supersaturation_data() -> Dict[str, np.ndarray]:
    ...
    rkoop_tau = np.float64(60.0)     # characteristic timescale [s]
...
def wrap_ice_supersaturation(func):
    ...
    arg_order = [
        ...
        "zsolqa", "zsolac", "zqxfg",
        "rtt", "ramin", "rthomo", "nssopt", "rkoop_tau", "ptsphy", "zepsec",
...
def run_ice_supersaturation():
    ...
    # ----- Build SDFG -----
    sdfg = ice_supersaturation_adjustment_dace.to_sdfg(simplify=False)
    ...
    compare_row_col_dicts(data, data_F, rtol=1e-12, atol=1e-12)
