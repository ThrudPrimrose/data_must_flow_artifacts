...
configs=(
    ...
    "-march=armv9-a+simd+nosve+nosve2  " "neon"
    # Force SVE2 (disable NEON fallback)
    "-march=armv9-a+sve2+sve+nosimd" "sve"
    # Force NEON only (disable SVE/SVE2) and use DaCe intrinsics (default=Neon)
    "-march=armv9-a+simd+nosve+nosve2 -D__DACE_USE_INTRINCS=0 -D__DACE_USE_SVE=0" "intrinsic_neon"
    # Force SVE only and use DaCe intrinsics (default=Neon)
    "-march=armv9-a+sve2+sve+nosimd -D__DACE_USE_INTRINCS=1 -D__DACE_USE_SVE=1" "intrinsic_sve"
    "-fno-tree-vectorize -fno-slp-vectorize" "no_vectorize"
)
