...
for kernel in KERNELS:
    ...
    for variant in BASE_VARIANTS:
        TARGET_FILENAMES.append(
            f"{kernel}_timings__{variant}singlecore.csv"
        )
        TARGET_FILENAMES.append(
            f"{kernel}_timings__{variant}singlecore.csv"
        )
        TARGET_FILENAMES.append(
            f"{kernel}__{variant}singlecore.csv"
        )
...
df_all = df_all[df_all["size"] == "37748736"]
print(df_all)
import re
...
is_dace_vec = summary["variant"].eq("dace")
...
plt.savefig("branchdependentwrite.pdf")
