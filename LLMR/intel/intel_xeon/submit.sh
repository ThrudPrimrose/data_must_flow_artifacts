#!/bin/bash
#SBATCH --job-name=llmr_intel_xeon_  # Job name
...
configs=(
    ...
    "-qopt-zmm-usage=high -diag-enable=vec -qopt-report=5 -qopt-report-phase=vec -qopt-report-file=${SCRIPT_DIR}/llmr_vec_report.optrpt" "force_width_512"
    "-no-vec" "no_vectorize"
)
...
for RUNMULTI in 0 1; do
    ...
    for ((i=0; i<${#configs[@]}; i+=2)); do
        ...
        cp ../../benchmark_llmr.py .

        # Run benchmark
        python3 benchmark_llmr.py

        # Remove script
        rm benchmark_llmr.py
    done
done