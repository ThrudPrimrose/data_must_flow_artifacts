...
for intrin in ["_dace_intrin__", ""]:
    paths = [
        #("gcc",  "amd_epyc",   "gcc/amd_epyc/exp_implementations_timings__singlecore.csv"),
        ("gcc",  "intel_xeon", f"gcc/intel_xeon/exp_implementations_timings__{intrin}singlecore.csv"),
        #("gcc",  "arm", "gcc/arm/exp_implementations_timings__singlecore.csv"),
        #("llvm", "amd_epyc",   "llvm/amd_epyc/exp_implementations_timings__singlecore.csv"),
        ("llvm", "intel_xeon", f"llvm/intel_xeon/exp_implementations_timings__{intrin}singlecore.csv"),
        #("llvm", "arm", "llvm/arm/exp_implementations_timings__singlecore.csv"),
        #("intel", "amd_epyc", "llvm/intel_xeon/exp_implementations_timings__singlecore.csv"),
        ("intel", "intel_xeon", f"llvm/intel_xeon/exp_implementations_timings__{intrin}singlecore.csv"),
        
    ]
    for vl in ["8", "16", "32", "64"]:
        for l, k, v in [("w_cpy", f"exp_implementations_std_veclen_{vl}_cpy", f"Auto + DaCe Vectorized (veclen={vl}, expl. copy)"),
                    ("no_cpy", f"exp_implementations_std_veclen_{vl}_no_cpy", f"Auto + DaCe Vectorized (veclen={vl}, no. copy)"),
                    ]:


            label_map = {
                "exp_implementations_std": "Auto Vectorized",
                k: v, 
            }
            ...
            # SDFGs to include
            # ------------------------
            alt_name = base_name + k[len("exp_implementations_std"):]
            ...
            color_map = {
            'exp_implementations_std': colors[1],
            f'exp_implementations_std_vectorized_static_veclen_{vl}_no_cpy': colors[1],
            f'exp_implementations_std_vectorized_static_veclen_{vl}_cpy': colors[2],
            }
            ...
            plt.tight_layout()
            plt.savefig(args.output_png.split(".png")[0] + "_" + l + f"_{vl}_{intrin}.png")
            print(f"Saved figure to {args.output_png}"  + "_" + l + f"_{vl}_{intrin}")
