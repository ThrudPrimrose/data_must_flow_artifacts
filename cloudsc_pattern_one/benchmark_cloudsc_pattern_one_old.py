...
# Parse arguments
# -----------------------------
parser = argparse.ArgumentParser(description="Run cloudsc_pattern_one DaCe benchmarks")
parser.add_argument(
    "--suffix",
    type=str,
    default="",
...
csv_filename = f"cloudsc_pattern_one_timings{('_' + args.suffix) if args.suffix else ''}_1_core.csv"
...
def build_vectorized_sdfg(base_sdfg, vec_width, insert_copies, suffix):
    ...
    name = f"cloudsc_pattern_one_vectorized_static_veclen_{vec_width}_{suffix}"
    sdfg.name = name
    sdfg.instrument = dace.dtypes.InstrumentationType.Timer
...
def build_aligned_arrays_and_sdfg(base_sdfg, S, vec_width):
    ...
    name = f"cloudsc_pattern_one_vectorized_static_veclen_{vec_width}_no_cpy_aligned"
    sdfg.name = name
    ...
    for i, N in enumerate([512, 1024, 2048, 4096, 8192, 16384]):
        ...
        for l in [8, 16, 32]:
            ...
            sdfg_vec, name = build_vectorized_sdfg(
                cloudsc_pattern_one_sdfg, vec_width=l, insert_copies=False, suffix="no_cpy"
            )
            ...
            sdfg_vec_cpy, name = build_vectorized_sdfg(
                cloudsc_pattern_one_sdfg, vec_width=l, insert_copies=True, suffix="cpy"
            )
            all_timings[name, size] = run_sdfg_multiple_times(
